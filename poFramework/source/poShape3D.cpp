...
poObject* poShape3D::duplicate( bool copyChildren, poObject* copyIntoObject )
{
	// either make new poShape2D or accept external one
	poShape3D* obj = (poShape3D*) copyIntoObject;
	if ( obj==NULL )
		obj = new poShape3D();
	
	// set poShape3D specific properties
	obj->fillColor = fillColor;
	obj->strokeColor = strokeColor;
	obj->pointList = pointList;
	obj->normalList = normalList;
	obj->textureCoordList = textureCoordList;
...
void poShape3D::drawFaces()
{
	...
	for( int f=0; f<faceList.size(); f++ )
	{
		...
		if ( useFaceColors )
			glColor4f( F.color.R, F.color.G, F.color.B, F.color.A*masterAlphaControl);
