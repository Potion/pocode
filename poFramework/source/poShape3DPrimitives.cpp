...
poMesh3D::poMesh3D()
{
	...
	registry.add( "numRows", &numRows );
	registry.add( "numColumns", &numColumns );
}
...
poPlane3D::poPlane3D( int rows, int columns, float w, float h )
{
	construct( rows, columns, w, h );
}
...
void		poPlane3D::init()
{
	construct( numRows, numColumns, width, height );
}
...
void		poSphere3D::init()
{
	construct( numRows, numColumns, radius );
}
...
void	poSphere3D::construct( int rows, int columns, float radius  )
{
	...
	for( int j=0; j<numRows; j++ )
	{
		for( int i=0; i<numColumns; i++ )
		{
			...
			pointList[N].set(  dcos(B)*dcos(A)*radius, dsin(B)*radius, dcos(B)*dsin(A)*radius );
		}
	}
...
poMesh3D* poShape3DMaker::makeRevolutionFromShape2D( poShape2D* S, float radialOffset, int numSlices, float startAngle, float endAngle, bool makeEndCaps )
{
	...
	for( int i=0; i<=S->pointList.size(); i++ )
	{
		...
		for( int j=0; j<numSlices; j++ )
		{
			...
			poPoint P;
			P.y = shapePoint.y;
			P.x = (shapePoint.x+radialOffset) * dcos(A);
			P.z = (shapePoint.x+radialOffset) * dsin(A);
