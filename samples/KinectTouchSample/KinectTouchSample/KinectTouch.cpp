...
KinectTouch::KinectTouch(int scr_x, int scr_y, 
                         ...
                         bool flip_vw, string ovr_img_path) {
    ...
    screen_table_x = scr_x;
    screen_table_y = scr_y;
...
void KinectTouch::run() {
	...
	int touchDepthMin = 12;
	int touchDepthMax = 22;
	int touchMinArea =  20;
	...
	while ( waitKey(1) != 27 ) {
		...
		for (unsigned int i=0; i<contours.size(); i++) {
            ...
            if ( contourArea(contourMat) > touchMinArea && contourMat.rows >= 6 ) {
                ...
                Point2f touchPoint(rrect.center.x - x, rrect.center.y + y);
                touchPoints.push_back(touchPoint);
                win->touchBegin( rrect.center.x - x, rrect.center.y + y , i );
			}
		}
